...
export function AdminWithdrawals() {
  ...
  const approveWithdrawalMutation = useMutation({
    mutationFn: async (withdrawalId: string) => {
      ...
      // Update withdrawal status and set approved_at timestamp
      ...
      const { data: currentWallet, error: walletError } = await supabase
        .from("user_wallets")
        .select("total_profit, daily_earnings")
        .eq("user_id", withdrawal.user_id)
        .single();

      if (walletError) throw walletError;

      const totalAvailable = (currentWallet.total_profit || 0) + (currentWallet.daily_earnings || 0);
      const newTotalProfit = Math.max(0, totalAvailable - withdrawal.amount);

      // Update wallet to deduct the withdrawn amount
      const { error: updateWalletError } = await supabase
        .from("user_wallets")
        .update({ 
          total_profit: newTotalProfit,
          daily_earnings: 0 // Reset daily earnings as we're transferring to total_profit and then deducting
        })
        .eq("user_id", withdrawal.user_id);
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Withdrawal has been approved and amount deducted from user's wallet.",
      });
    },
  });
