-- Fix the double deposit issue in process_nft_deposit_with_shared_timer function
-- The issue is that the function is creating duplicate NFT deposit entries

-- Drop the existing trigger first
DROP TRIGGER IF EXISTS process_nft_deposit_trigger ON public.deposits;

-- Drop and recreate the function with proper logic to prevent double entries
DROP FUNCTION IF EXISTS public.process_nft_deposit_with_shared_timer();
...
CREATE TRIGGER process_nft_deposit_trigger
  AFTER UPDATE ON public.deposits
  FOR EACH ROW
  EXECUTE FUNCTION public.process_nft_deposit_with_shared_timer();

-- Clean up existing duplicate NFT deposits for current user
...
-- Update batch numbers to be sequential
WITH numbered_deposits AS (
  SELECT id, user_id, 
         ROW_NUMBER() OVER (PARTITION BY user_id ORDER BY deposit_date ASC) as new_batch_number
  FROM public.nft_deposits
)
UPDATE public.nft_deposits 
SET batch_number = numbered_deposits.new_batch_number
FROM numbered_deposits
WHERE nft_deposits.id = numbered_deposits.id;